import { createConnection, createWidget, withAmplitude, withPayments } from '@livechat/widget-core-sdk';
...
var AVAILABLE_COMPONENT_TYPES = Object.keys(SectionComponentType).map(function (name) {
  return SectionComponentType[name];
});
var componentValidators = (_componentValidators = {}, _componentValidators[SectionComponentType.Button] = function (_ref) {
  var data = _ref.data;
  if (!data || !data.label) {
...
}, _componentValidators[SectionComponentType.Link] = function (_ref2) {
  var data = _ref2.data;
  if (!data || !data.url) {
    throw new Error("The link component must have a 'url' property");
  }
}, _componentValidators[SectionComponentType.Title] = function (_ref3) {
  var data = _ref3.data;
  if (!data || !data.title) {
...
function DetailsWidget(connection) {
  ...
  var widget = withAmplitude(withRichMessages(withCustomerProfile(withTheme(withPayments(base)))));
  return widget;
}
...
export { SectionComponentType, createDetailsWidget, createFullscreenWidget, createMessageBoxWidget, createSettingsWidget, withCustomerProfile };
